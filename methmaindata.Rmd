...
This script creates a bar plot which displays the percentage of duplicate reads present in each of our population DNA samples. As the number of total reads rises, the percentage of duplicate reads rises non-linearly. 
...
This script creates a bar plot which displays the mean read length (in bp) of each population DNA sample. We expect the average read length to be ~150 bp. This read length optimises the sequencing process by balancing reading the most base pairs in each fragment while not creating poor quality reads. This therefore, is the most cost efficient read length. Each of our samples average read length is very close to this number which therefore indicates cost efficiency and high coverage of the methylation profiling.
...
This script creates a stacked barplot which displays what percentage of bases are removed by each data processing step, the percentage of bases removed due to overlapping reads and the percentage of usable bases left in each of the population DNA samples. While overall the loss of bases was low for most samples, there were evidently problems mapping reads from the Clear R1 and Lilac R3 samples. We know that the problem is caused due to mapping difficulties as these samples did not contain a large number of duplicate reads. 
