...
head(CvGpromoterschrdf)
CvGup <- subset(CvGpromoterschrdf,qvalue<0.05&meth.diff>0)
CvGupT <- table(CvGup$gene_name)
CvGupT <- CvGupT[which(names(CvGupT) !="uncharacterised")]
head(CvGupT[order(-CvGupT)],20)
CvGupTnames <- names(head(CvGupT[order(-CvGupT)],20))

CvGdown <- subset(CvGpromoterschrdf,qvalue<0.05&meth.diff<0)
CvGdownT <- table(CvGdown$gene_name)
CvGdownT <- CvGdownT[which(names(CvGdownT) !="uncharacterised")]
head(CvGdownT[order(-CvGdownT)],20)
CvGdownTnames <- names(head(CvGdownT[order(-CvGdownT)],20))
gn <- union(CvGupTnames,CvGdownTnames)
sapply(gn, function(g){
  up <- CvGupT[which(names(CvGupT)%in%g)]
  down <- CvGdownT[which(names(CvGdownT)%in%g)]
  return(c(up,down))
})
